...
class Skill(Model, table=True):
    ...
    scenario : "Scenario" = db.backref('skills')

    competencies : List["Competency"] = db.backref('skill')
    roles : List["Role"] = db.backref('skill')
            

class Contributor(Model, table=True):
    name : str = db.column()
    scenario_id : int = db.foreign_key(Scenario.id)
    scenario : "Scenario" = db.backref('contributors')

    competencies : List["Competency"] = db.backref('contributor')
...
class Competency(Model, table=True):
    ...
    skill : Skill = db.backref(Skill.competencies)
    contributor : Contributor = db.backref(Contributor.competencies)
...
class Role(Model, table=True):
    ...
    skill : Skill = db.backref('roles')
    project : "Project" = db.backref("roles")
...
class Project(Model, table=True):
    ...
    scenario : "Scenario" = db.backref(Scenario.projects)
    roles : List[Role] = db.backref('project')
